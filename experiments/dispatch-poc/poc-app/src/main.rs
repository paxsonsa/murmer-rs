...
use poc_framework::{Actor, ActorContext, Message, RemoteMessage};
...
mod tests {
    ...
    async fn test_remote_actor_via_simulated_wire() {
        ...
        let receiver_handle = tokio::spawn(poc_framework::run_node_receiver(
            recv_receptionist,
            wire_rx,
            resp_tx,
        ));
    ...
    async fn test_node_pruning() {
        ...
        assert!(receptionist.lookup::<CounterActor>("counter/local-1").is_some());
        assert!(receptionist.lookup::<CounterActor>("counter/remote-1").is_some());
        assert!(receptionist.lookup::<CounterActor>("counter/remote-2").is_some());
        ...
        assert!(receptionist.lookup::<CounterActor>("counter/local-1").is_some());
        assert!(receptionist.lookup::<CounterActor>("counter/remote-1").is_none());
        assert!(receptionist.lookup::<CounterActor>("counter/remote-2").is_none());
    }
    ...
    async fn test_restart_limit_permanent_death() {
        ...
        assert!(lookup.is_none(), "actor should be gone after restart limit exceeded");

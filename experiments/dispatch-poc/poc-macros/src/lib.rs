...
use quote::quote;
use syn::{FnArg, ImplItem, ItemImpl, ReturnType, parse_macro_input};
...
pub fn handlers(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let handler_impls = handler_infos.iter().map(|h| {
        ...
        quote! {
            impl poc_framework::Handler<#msg_type> for #self_ty {
                fn handle(
                    &mut self,
                    ctx: &poc_framework::ActorContext<Self>,
                    state: &mut <Self as poc_framework::Actor>::State,
                    message: #msg_type,
                ) -> <#msg_type as poc_framework::Message>::Result {
    ...
    let match_arms = handler_infos.iter().map(|h| {
        let msg_type = &h.msg_type;
        quote! {
            <#msg_type as poc_framework::RemoteMessage>::TYPE_ID => {
                let (msg, _): (#msg_type, _) =
                    poc_framework::__reexport::bincode::serde::decode_from_slice(
                        payload,
                        poc_framework::__reexport::bincode::config::standard(),
                    )
                    .map_err(|e| poc_framework::DispatchError::DeserializeFailed(
                        e.to_string(),
                    ))?;
                let result = <Self as poc_framework::Handler<#msg_type>>::handle(
                    self, ctx, state, msg,
                );
                poc_framework::__reexport::bincode::serde::encode_to_vec(
                    &result,
                    poc_framework::__reexport::bincode::config::standard(),
                )
                .map_err(|e| poc_framework::DispatchError::SerializeFailed(
                    e.to_string(),
                ))
            }
        }
    ...
    let remote_dispatch = quote! {
        impl poc_framework::RemoteDispatch for #self_ty {
            fn dispatch_remote(
                &mut self,
                ctx: &poc_framework::ActorContext<Self>,
                state: &mut <Self as poc_framework::Actor>::State,
                message_type: &str,
                payload: &[u8],
            ) -> Result<Vec<u8>, poc_framework::DispatchError> {
                match message_type {
                    #(#match_arms)*
                    __other => Err(poc_framework::DispatchError::UnknownMessageType(
                        __other.to_string(),
                    )),
                }
            }

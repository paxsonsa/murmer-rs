...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        eprintln!("Usage:");
        eprintln!(
            "  {} server <port>                    - Start a server node",
            args[0]
        );
        eprintln!(
            "  {} client <server-addr>             - Start a client node that connects to server",
            args[0]
        );
        eprintln!(
            "  {} ping <node1-port> <node2-addr>   - Connect two nodes and ping between them",
            args[0]
        );
        eprintln!();
        eprintln!("Examples:");
...
async fn run_server(port: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bind_addr = NodeAddress::new(IpAddr::V4(Ipv4Addr::LOCALHOST), port);
    let node_id = NodeId::from_socket_addr(bind_addr.to_socket_addr());
    let config = NodeServerConfig::new_with_generated_cert(bind_addr, &node_id)?;

    let server = NodeServer::new(config, node_id.clone()).await?;
...
async fn run_client(server_addr: NodeAddress) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bind_addr = NodeAddress::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 0);
    let node_id = NodeId::from_socket_addr(bind_addr.to_socket_addr());
    ...
    let server_id = murmer_core::node::NodeId::from_socket_addr(server_addr.to_socket_addr());

    match server.connect_to_node(server_id.clone(), server_addr).await {
...
async fn run_ping_test(
    node1_port: u16,
    node2_addr: NodeAddress,
) -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "Running ping test between node1 (port {}) and node2 ({})",
        node1_port, node2_addr
    );

    // Create node 1
    let node1_id = NodeId::from_socket_addr(
        NodeAddress::new(IpAddr::V4(Ipv4Addr::LOCALHOST), node1_port).to_socket_addr(),
    );
    let node1_config = NodeServerConfig::new_with_generated_cert(
        NodeAddress::new(IpAddr::V4(Ipv4Addr::LOCALHOST), node1_port),
        &node1_id,
    )?;
    let node1_server = NodeServer::new(node1_config, node1_id.clone()).await?;
    let node1_actual_addr = node1_server.local_addr()?;

    // Create node 2
    let node2_id = NodeId::from_socket_addr(node2_addr.to_socket_addr());
    let node2_config = NodeServerConfig::new_with_generated_cert(node2_addr, &node2_id)?;
    let node2_server = NodeServer::new(node2_config, node2_id.clone()).await?;
    ...
    node1_server
        .connect_to_node(node2_id.clone(), node2_actual_addr)
        .await?;
    node2_server
        .connect_to_node(node1_id.clone(), node1_actual_addr)
        .await?;
    ...
    match node1_server
        .ping_node(&node2_id, "Hello from Node1!".to_string())
        .await
    {
        Ok(response) => {
    ...
    match node2_server
        .ping_node(&node1_id, "Hello from Node2!".to_string())
        .await
    {
        Ok(response) => {

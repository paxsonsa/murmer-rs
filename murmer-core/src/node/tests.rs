use crate::node::{NodeAddress, NodeId, NodeServer, NodeServerConfig, generate_self_signed_cert};
...
async fn test_two_node_ping_pong() {
    ...
    // Create node IDs
    let node1_id = NodeId::from_socket_addr("127.0.0.1:0".parse().unwrap());
    let node2_id = NodeId::from_socket_addr("127.0.0.1:0".parse().unwrap());
    ...
    // Create both servers
    let server1 = NodeServer::new(node1_config, node1_id)
        .await
        .expect("Failed to create server1");
    let server2 = NodeServer::new(node2_config, node2_id)
        .await
    ...
    server2
        .connect_to_node(server1.node_id().clone(), node1_addr)
        .await
        .expect("Failed to connect node2 to node1");

    // Connect node1 to node2
    server1
        .connect_to_node(server2.node_id().clone(), node2_addr)
        .await
    ...
    println!(
        "Received pong from {}: {}",
        pong_response.from_node, pong_response.response
    );
...
async fn test_node_id_generation() {
    let id1 = NodeId::from_socket_addr("127.0.0.1:8001".parse().unwrap());
    let id2 = NodeId::from_socket_addr("127.0.0.1:8002".parse().unwrap());
...
async fn test_certificate_generation() {
    let node_id = NodeId::from_socket_addr("127.0.0.1:9000".parse().unwrap());
    let (cert_chain, _private_key) =
        generate_self_signed_cert(&node_id).expect("Failed to generate certificate");
